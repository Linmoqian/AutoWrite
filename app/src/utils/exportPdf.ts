...
export function renderPrintView(data: ExportData): void {
  ...
  let html = PRINT_CSS;
  html += buildTitlePage(data);
  html += buildTableOfContents(data);
  html += buildWorldView(data);
  html += buildCharacters(data);
  html += buildChapters(data);
...
function buildTitlePage(data: ExportData): string {
...
function buildTableOfContents(data: ExportData): string {
  if (data.outline.length === 0) return "";
  let html = `<div class="toc"><h1>目录</h1><ul>`;
...
function buildWorldView(data: ExportData): string {
  if (!data.novel.worldView) return "";
  return `<h1>世界观</h1>${renderMdBody(data.novel.worldView)}`;
}

function buildCharacters(data: ExportData): string {
  if (!data.novel.characters) return "";
  return `<h1>角色</h1>${renderMdBody(data.novel.characters)}`;
}

function buildChapters(data: ExportData): string {
  let html = "";
